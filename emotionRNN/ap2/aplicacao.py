...
class HOGFeatureExtractorMelhorado:
    """Extrator HOG melhorado compatível com o modelo treinado"""
    
    @staticmethod
    def calcularGradiente(imagem):
        # Suavização antes do cálculo do gradiente (igual ao treinamento)
    ...
    @staticmethod
    def calcularHistogramaCelula(imagens, tamanho_imagem=(64,64), tamanho_bloco=2, tamanho_celula=6, bins=12):
        """HOG melhorado com normalização robusta - compatível com o modelo treinado"""
        hog_vetores = []
        ...
        for img in tqdm(imagens, desc="Enhanced HOG Features"):
            ...
            # Normalização da imagem (igual ao treinamento)
            ...
            # Interpolação bilinear para bins (igual ao treinamento)
                    ...
                    for y_cell in range(tamanho_celula):
                        for x_cell in range(tamanho_celula):
                            if y_cell < mag_celula.shape[0] and x_cell < mag_celula.shape[1]:
                                ...
                                # Interpolação bilinear entre bins
            ...
            # Normalização L2-Hys melhorada (igual ao treinamento)
...
class EmotionVideoDetector:
    def __init__(self, model_path, scalers_path=None):
        ...
        # Classes de emoção (atualizadas conforme o modelo)
        ...
        self.mobilenet = tf.keras.applications.MobileNetV2(
            ...
            include_top=False, 
            pooling='avg', 
            input_shape=(48,48, 3)  # Tamanho atualizado
        )
        # Fine-tuning: descongelar algumas camadas finais (igual ao treinamento)
        ...
        # Scalers para normalização (serão criados se não fornecidos)
        self.hog_scaler = StandardScaler()
        self.cnn_scaler = StandardScaler()
        self.scalers_fitted = False
        
        # Carregar scalers se disponíveis
    ...
    def processar_frame_individual(self, frame):
        ...
        # Detecção de face com múltiplas escalas
        ...
        if len(faces) > 0:
            ...
            # Expande a região da face ligeiramente
    ...
    def extrair_todos_frames(self, video_path):
        """Extrai todos os frames do vídeo com processamento melhorado"""
